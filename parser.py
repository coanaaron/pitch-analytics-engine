...
def calculate_pitch_metrics(df: pd.DataFrame, pitcher_name: str) -> pd.DataFrame:
    ...
    filtered_df = df[df['Pitcher'] == pitcher_name]
...
def start_grade_calculator(pitcher_df: pd.DataFrame) -> str:
    """
    ...
    return grade_mapping.get(integer_grade, "F")




def calculate_box_score(df: pd.DataFrame, pitcher_name: str, is_starter: bool = False) -> pd.DataFrame:
    """
    Filters data for a specific pitcher, applies trackman data filtering,
    ...
    if filtered_df.empty:
        return pd.DataFrame(columns=['Pitcher', 'IP', 'H', 'ER', '2B', '3B', 'HR', 'BB', 'HBP', 'K', 'Pitches', 'Start_Grade'])
...
def get_metadata(df: pd.DataFrame, pitcher_name: str) -> list:
    """
...
box_score = calculate_box_score(df, 'Kruzan, Marcus', True)
print(box_score.to_string(index = False))

metadata = get_metadata(df, 'Kruzan, Marcus')
print(metadata)
